...
from token_merge import bipartite_soft_matching_with_idx
        ...
        if self.token_merge:
            ...
            if condition and self.fusion_token > 0:
                if x.ndim == 4:
                    ...
                    merge, source_idx, target_idx = bipartite_soft_matching_with_idx(x_flat, remove_tokens, class_token=False)
                    x_merged = merge(x_flat)  # (B, R, C)
                    self.x_merged = x_merged  # ← Token Merge 이후 시각화를 위한 저장

                    # 시각화용 정보 저장
                    self.merge_source_idx = source_idx  # merge된 위치 인덱스
                    self.merge_target_idx = target_idx  # merge된 대상 인덱스
                    self.original_shape = (H_ori, W_ori)  # 나중에 복원용
